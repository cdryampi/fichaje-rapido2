...
def test_invitado_can_view_only_whitelisted(db_session):
    ...
    guest = User(email='guest@test', name='guest', role=Role.invitado)
    u1 = User(email='u1@test', name='u1', role=Role.employee, group=g, area=area)
    u2 = User(email='u2@test', name='u2', role=Role.employee, group=g, area=area)
    ...
    assert can_view_user(guest, u1) is True
    assert can_view_user(guest, u2) is False
...
def test_responsable_scope_group_can_and_cannot(db_session):
    ...
    resp = User(email='resp@test', name='resp', role=Role.responsable, group=g1, area=a)
    e1 = User(email='e1@test', name='e1', role=Role.employee, group=g1, area=a)
    e2 = User(email='e2@test', name='e2', role=Role.employee, group=g2, area=a)
